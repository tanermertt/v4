...
# Dışarıdan alınan gelir vergisi fonksiyonu
...
def hesapla():
    try:
        ...
        yardimlar = (
            ...
            yemek_yardimi + sosyal_yardim + ise_devam_tesvik_primi + ekstra_prim +
            brüt_ikramiye + uretim_destek_primi + yillik_izin_kazanci + yol_yardimi_toplam
        )
        ...
        gelir_vergisi_matrahi = toplam_kazanc - (
            sgk_primi + isssizlik_primi + yol_yardimi + cocuk_yardimi +
            son_yevmiyesi + isveren_bes_sigorta + (calisan_gun * 264) + engelli_indirimi
        )
        ...
        toplam_vergi = max(0, kumulatif_vergi - kumule_gelir_vergisi - istisna)

        damga_vergisi_matrahi = toplam_kazanc - (calisan_gun * 264)
        damga_vergisi = max(0, (damga_vergisi_matrahi * 0.00759) - 197.38)
...
if st.button("Hesapla"):
    ...
    if sonuclar:
        ...
        st.markdown(f"<h3 style='color:red; font-weight:bold'>{sonuclar['Net Maaş']:.2f} TL</h3>", unsafe_allow_html=True)
